...
libsodium_installation(){
	...
	if [[ ! -f ${libsodium_folder}/libsodium-${Libsodiumr_ver}-RELEASE.tar.gz ]]; then
		echo -e "${Error} ${RedBG} libsodium download FAIL ${Font}"
		exit 1
	fi
	tar xf libsodium-${Libsodiumr_ver}-RELEASE.tar.gz && cd libsodium-${Libsodiumr_ver}-RELEASE
...
#basic install	
	...
	cd ${shadowsocks_install_folder} && git clone -b manyuser https://github.com/NimaQu/shadowsocks.git 
